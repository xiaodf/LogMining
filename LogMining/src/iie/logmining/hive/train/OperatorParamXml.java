...
public class OperatorParamXml {
	...
	public List<Map> parseStdinXml(String stdinXml) throws Exception {
		...
		// 以下为spark streaming 用到的参数
		String matchRate = null;
		String logMessageField = null;
		String zkquorum = null;
		String groupID = null;
		String thriftURI = null;
		String outputDBName = null;
		String outputMatchTable = null;
		String outputWarningTable = null;
		String outputNotagTable = null;
		List<String> ips = new ArrayList<String>();
		List<String> topicAndThreads = new ArrayList<String>();
		...
		while (iter1.hasNext()) {
			...
			if ("operator".equals(node2.getName())) {
				...
				if ("AddLabel".equals(node2.attributeValue("name"))) {
					Iterator iter2 = node2.elementIterator();
					while (iter2.hasNext()) {
						Element node3 = (Element) iter2.next();
						if ("parameter".equals(node3.getName())) {
				...
				if ("LogMergeAndMining".equals(node2.attributeValue("name"))) {
					...
					while (iter2.hasNext()) {
						...
						if ("parameter".equals(node3.getName())) {
							...
							if ("stepSize".equals(node3.attributeValue("name"))) {
				...
				if ("LogMatcherFinal".equals(node2.attributeValue("name"))) {
					Iterator iter2 = node2.elementIterator();
					while (iter2.hasNext()) {
						Element node3 = (Element) iter2.next();
						if ("parameter".equals(node3.getName())) {
							if ("matchRate"
								...
								matchRate = node3.getText();
								map.put("matchRate", matchRate);
							}
							if ("logMessageField".equals(node3
									.attributeValue("name"))) {
								logMessageField = node3.getText();
								map.put("logMessageField", logMessageField);
							}
							if ("zkquorum".equals(node3.attributeValue("name"))) {
								zkquorum = node3.getText();
								map.put("zkquorum", zkquorum);
							}
							if ("groupID".equals(node3.attributeValue("name"))) {
								groupID = node3.getText();
								map.put("groupID", groupID);
							}
							if ("thriftURI"
									.equals(node3.attributeValue("name"))) {
								thriftURI = node3.getText();
								map.put("thriftURI", thriftURI);
							}
							if ("outputDBName".equals(node3
									.attributeValue("name"))) {
								outputDBName = node3.getText();
								map.put("outputDBName", outputDBName);
							}
							if ("outputMatchTable".equals(node3
									.attributeValue("name"))) {
								outputMatchTable = node3.getText();
								map.put("outputMatchTable", outputMatchTable);
							}
							if ("outputWarningTable".equals(node3
									.attributeValue("name"))) {
								outputWarningTable = node3.getText();
								map.put("outputWarningTable",
										outputWarningTable);
							}
							if ("outputNotagTable".equals(node3
									.attributeValue("name"))) {
								outputNotagTable = node3.getText();
								map.put("outputNotagTable", outputNotagTable);
							}
							if ("topicAndThreads".equals(node3
									.attributeValue("name"))) {
								Iterator iter3 = node3.elementIterator();
								while (iter3.hasNext()) {
									Element node4 = (Element) iter3.next();
									if ("topic".equals(node4.getName())) {
										topicAndThreads.add(node4.getText());
									}
									topicMap.put("topicAndThreads",
											topicAndThreads);
									System.out.println(topicAndThreads);
								}
							}
						}
					}
