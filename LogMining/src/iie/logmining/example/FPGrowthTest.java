...
public class FPGrowthTest {
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
			System.err
					.println("Usage: JavaFPGrowth <input_file> [minSupport] [numPartition]");
			System.exit(1);
		}
		inputFile = args[0];
		if (args.length >= 2) {
		...
		List<List<String>> list = new ArrayList<List<String>>();// 用来存放事务
		SparkConf sparkConf = new SparkConf().setAppName("FPGrowthTest");
		JavaSparkContext sc = new JavaSparkContext(sparkConf);
		JavaRDD<List<String>> transactions = sc.parallelize(list);

		FPGrowthModel<String> model = new FPGrowth().setMinSupport(minSupport)
				.setNumPartitions(numPartition).run(transactions);

		for (FPGrowth.FreqItemset<String> s : model.freqItemsets().toJavaRDD()
				.collect()) {
