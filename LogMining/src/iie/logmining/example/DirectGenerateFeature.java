 ...
 * 
 *  spark-submit --class iie.logmining.spark.train.DirectGenerateFeature  
 *  --master yarn-client /home/xdf/test.jar  xdf log3 log4  0.5
 *  建表语句：
 *  CREATE TABLE  log4(label STRING,feature STRING,id STRING) 
...
public class DirectGenerateFeature implements Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		String addIDOutTable = parameters.get(0).get("addIDOutTable")
				.toString(); // 输入表
		String featureTable = parameters.get(0).get("featureTable").toString(); // 输出表
		...
		// 获得节点邻接矩阵
		List<String> vertexList = lcsArr.repartition(1).collect();
		int rowCount = (int) vector.repartition(1).count();
		...
		for (int i = 0; i < vertexList.size(); i++) {
			...
			if (temp[2].equals("1")) {
				rowIdMap.put(temp[0], temp[3]);
			}
		}

		// 获得每行数据（行号，信息）
		List<String> rowVector = vector.repartition(1).collect();
		...
		for (int i = 0; i < label_set_list.size(); i++) {
			...
			String LabelFeature = "Label" + i;
			if (feature.length() > 2) {
				// 去掉"P,"
				feature = feature.substring(2);
			}
			List<Object> temp = new ArrayList<Object>(3);
			temp.add(LabelFeature);
			temp.add(feature);
			temp.add(docId);
			String sourceLogID = "";
			for (int k = 0; k < list.size(); k++) {
				sourceLogID += rowIdMap.get(list.get(k));
			}
			temp.add(sourceLogID);
